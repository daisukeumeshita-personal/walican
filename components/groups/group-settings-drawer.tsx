...
import { updateGroup, deleteGroup, addMember, removeMember, cancelInvitation, type GroupActionState } from '@/app/actions/groups'
    ...
    >
      ...
      <div className="flex gap-2">
        <input
          ...
          required
          className="flex-1 px-3 py-2.5 rounded-xl border border-border bg-card text-sm placeholder:text-muted/40 focus:outline-none focus:ring-2 focus:ring-foreground/10 transition-colors"
        />
        <SubmitButton pendingText="送信中..." className="shrink-0">招待</SubmitButton>
      </div>
    </form>
  )
}
  ...
  return (
    <Drawer.Root open={open} onOpenChange={setOpen} direction="right">
      <Drawer.Trigger asChild>
        <button className="text-muted hover:text-foreground transition-colors shrink-0 cursor-pointer" title="グループ設定">
      ...
      <Drawer.Portal>
        ...
        <Drawer.Content className="fixed right-0 top-0 bottom-0 z-50 flex flex-col w-full max-w-sm bg-card border-l border-border/60 shadow-xl focus:outline-none">
          <div className="flex items-center justify-between px-5 py-4 border-b border-border/60 shrink-0">
            ...
            <button onClick={() => setOpen(false)} className="text-muted hover:text-foreground transition-colors cursor-pointer">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <section className="flex flex-col gap-3">
                      ...
                      : isOwner && (
                          confirmRemoveId === m.user_id ? (
                            <div className="flex gap-1 shrink-0">
                              ...
                              <Button variant="ghost" className="text-xs px-2 py-1" onClick={() => setConfirmRemoveId(null)}>
                                取消
                              </Button>
                            </div>
                          ) : (
              ...
              {invitations.length > 0 && (
                <div className="flex flex-col gap-1 pt-1 border-t border-border/40">
                  <p className="text-[11px] text-muted uppercase tracking-wider mb-1">招待中</p>
                  {invitations.map(inv => (
                    <div key={inv.id} className="flex items-center gap-3 py-1.5">
                      <Avatar name={inv.email} className="w-8 h-8 !rounded-full shrink-0" />
                      <p className="text-sm text-muted flex-1 truncate">{inv.email}</p>
                      {isOwner && (
                        confirmCancelId === inv.id ? (
                          <div className="flex gap-1 shrink-0">
                            ...
                            >
                              {cancellingId === inv.id ? '取消中...' : '取消'}
                            </Button>
                            <Button variant="ghost" className="text-xs px-2 py-1" onClick={() => setConfirmCancelId(null)}>
                              戻る
                            </Button>
                          </div>
                        ) : (
              ...
              {canInvite && <InviteForm groupId={groupId} />}
            </section>
              ...
              <section className="flex flex-col gap-3">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    maxLength={50}
                    className="flex-1 px-3 py-2.5 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-foreground/10 transition-colors"
                  />
                  <Button onClick={handleSaveName} disabled={saving || name.trim() === groupName || !name.trim()}>
                    {saving ? '保存中...' : '保存'}
                  </Button>
                </div>
              </section>
              ...
              <section className="flex flex-col gap-3">
                ...
                {confirmDelete ? (
                  <div className="flex flex-col gap-2">
                    <p className="text-sm">本当に削除しますか？この操作は取り消せません。</p>
                    <div className="flex gap-2">
                  ...
                  <button onClick={() => setConfirmDelete(true)} className="text-sm text-danger hover:underline text-left cursor-pointer">
                    このグループを削除する
                  </button>
                )}
              </section>
